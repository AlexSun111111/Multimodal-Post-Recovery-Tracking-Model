...
def augment_seg(img_aug, img, seg, num_classes):
    seg = mask_to_onehot(seg, num_classes)
    aug_det = img_aug.to_deterministic() 
    image_aug = aug_det.augment_image(img.astype(np.float32))     #img.astype(np.float32)

    segmap = ia.SegmentationMapOnImage( seg , nb_classes=np.max(seg)+1 , shape=img.shape )
    segmap_aug = aug_det.augment_segmentation_maps( segmap )
    segmap_aug = segmap_aug.get_arr_int()
    segmap_aug = np.argmax(segmap_aug, axis=-1)
...
class Synapse_dataset(Dataset):
    def __init__(self, base_dir, list_dir, split, img_size, num_classes, norm_x_transform=None, norm_y_transform=None):
    ...
    def __getitem__(self, idx):
        if self.split == "train" or self.split == "Gu_Lou_train":
            slice_name = self.sample_list[idx].strip('\n')
            data_path = os.path.join(self.data_dir, slice_name+'.npz')
            data = np.load(data_path)
            image, label = data['image'], data['label']
            # if self.split == "Gu_Lou_train":
            #     image = image[104:616, 384:896]
            #     label = label[104:616, 384:896]
