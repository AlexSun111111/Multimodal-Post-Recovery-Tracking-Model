...
def trainer_synapse(args, model, snapshot_path):     #训练神经元
    best_loss = 100
    ...
    db_train = Synapse_dataset(base_dir=args.root_path, list_dir=args.list_dir, split=args.train_split, img_size=args.img_size, num_classes=args.num_classes,
    ...
    db_test = Synapse_dataset(base_dir=args.test_path, split=args.test_split, list_dir=args.list_dir, img_size=args.img_size, num_classes=args.num_classes)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # for i in range(9):
            #     a = label_batch[i].numpy()
            #     b = image_batch[i].numpy()
            ...
            logging.info('iteration %d : loss : %.10f, loss_ce: %.10f, loss_dice: %.10f' % (iter_num, loss.item(), loss_ce.item(), loss_dice.item()))
        ...
        if epoch_num >= int(max_epoch / 2) and (epoch_num + 1) % eval_interval == 0:
            ...
            # if mean_dice > best_mean_dice and mean_hd95 < best_mean_hd95:
